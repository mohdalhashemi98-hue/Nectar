...
import { ArrowLeft, Send, Phone, MoreVertical, Image, Paperclip, Smile, CheckCheck, Check } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="bg-card border-b border-border p-4 flex items-center gap-4 sticky top-0 z-10">
        ...
        <div className="flex items-center gap-3 flex-1">
          <div className="relative">
            <div className="w-12 h-12 rounded-full bg-gradient-mazaadi flex items-center justify-center text-white font-bold">
              {conversation.avatar}
            </div>
            {conversation.online && (
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div className="flex items-center justify-center">
          <span className="px-4 py-1.5 rounded-full bg-muted text-xs text-muted-foreground">
            Today
          </span>
        </div>
            ...
            <div className={`max-w-[80%] ${msg.sender === 'user' ? 'order-2' : 'order-1'}`}>
              <div
                className={`px-4 py-3 rounded-2xl ${
                  msg.sender === 'user'
                    ? 'bg-gradient-mazaadi text-white rounded-br-md'
              ...
              <div className={`flex items-center gap-1 mt-1 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                {msg.sender === 'user' && (
                  <CheckCheck className="w-3.5 h-3.5 text-mazaadi-primary" />
                )}
              </div>
            </div>
          </div>
        ...
        <div className="flex items-end gap-3">
          ...
          <div className="flex-1 relative">
            <input
              ...
              className="w-full px-4 py-3 pr-12 rounded-2xl bg-muted border border-border focus:border-mazaadi-primary/50 focus:outline-none transition-colors text-foreground placeholder:text-muted-foreground"
            />
          ...
          <button
            ...
            className={`w-12 h-12 rounded-full flex items-center justify-center transition-all ${
              message.trim()
                ? 'bg-gradient-mazaadi text-white shadow-lg shadow-mazaadi-primary/30 hover:shadow-mazaadi-primary/50'
